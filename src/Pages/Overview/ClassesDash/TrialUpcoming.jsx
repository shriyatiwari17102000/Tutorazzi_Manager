...
const TrialUpcoming = (props) => {
  ...
  return (
    <div className={classes.cont}>
       ...
       {data?.start_time &&  <p style={{fontSize:'14px'}}><Moment format="DD/MM/YYYY">{data?.start_time}</Moment></p>}
        </div>
        <div className={classes.inn_div2}>
            <p className={classes.tab}>1hr</p>
           {data.class_type === "Trial" &&  <p className={classes.tab}>Trial</p>}
        </div>
      {data?.start_time && data?.end_time && <p className={classes.p1} style={{marginBottom:"10px"}}> <Moment format="hh:mm A">{data?.start_time}</Moment> to <Moment format="hh:mm A">{data?.end_time}</Moment></p>}
